...
function getInterpolatedTransform(arr, index){

	if(index >= arr.length-1){
		return arr[index];
	} else {
		var floored = ~~(index);
		var lower = arr[floored];
		var higher = arr[floored+1];

		var fraction = index - floored;
		...
		for(var i in lower){
			...
			interpolated[i] = (a * (1.0 - fraction)) + (b * fraction);
		}
